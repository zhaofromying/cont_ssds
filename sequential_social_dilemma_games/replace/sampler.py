...
class AsyncSampler(threading.Thread, SamplerInput):
    ...
    def get_data(self):
        ...
        rollout = self.queue.get(timeout=6000.0)
...
def _env_runner(worker, base_env, extra_batch_callback, policies,
    ...
    cf_rewards = {}
    agents_num = len(base_env.envs[0].agents)
    for i in range(agents_num):
        cf_rewards['agent-'+ str(i)] = 0
    while True:
        perf_stats.iters += 1
        t0 = time.time()
        ...
        cf_envs = [copy.deepcopy(base_env) for _ in range(agents_num)]
        ...
        for i in range(agents_num):
            ...
            for j in range(predict_steps):
                ...
                cf_active_envs, to_eval, outputs = _process_observations(
                    worker, cf_envs[i], policies, batch_builder_pool, mok_episodes,
                ...
                cf_actions = _process_policy_eval_results(
                    to_eval, eval_results, mok_episodes, cf_active_envs,
                    off_policy_actions, policies, clip_actions)
        ...
        for agent_id, raw_obs in agent_obs.items():
            ...
            if (last_observation is not None and infos[env_id].get(
                ...
                episode.batch_builder.add_values(
                    ...
                    actions=episode.last_action_for(agent_id),
                    rewards=rewards[env_id][agent_id],
                    cf_rewards=cf_rewards[agent_id],
                    prev_actions=episode.prev_action_for(agent_id),
                    prev_rewards=episode.prev_reward_for(agent_id),
                    dones=(False if (no_done_at_end
                                     or (hit_horizon and soft_horizon)) else
