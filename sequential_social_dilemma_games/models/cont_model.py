...
class ContModel(RecurrentTFModelV2):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        """
        super(ContModel, self).__init__(obs_space, action_space, num_outputs, model_config, name)
        ...
        self.contribute_divergence_measure = model_config["custom_options"][
            "contribute_divergence_measure"
        ]
        ...
        self._contribute_reward = None
        self._true_one_hot_actions = None

        self.cont_encoder_model = self.create_cont_encoder_model(obs_space, model_config)
        self.register_variables(self.cont_encoder_model.variables)
        # self.moa_encoder_model.summary()

        # now output two heads, one for action selection and one for the prediction of other agents
        inner_obs_space = self.cont_encoder_model.output_shape[0][-1]
        ...
        self.train_cont_only_when_visible = model_config["custom_options"][
            "train_cont_only_when_visible"
        ]
        self.contribute_only_when_visible = model_config["custom_options"][
            "contribute_only_when_visible"
        ]
        self.cont_weight = model_config["custom_options"]["moa_loss_weight"]
    ...
    @staticmethod
    def create_cont_encoder_model(obs_space, model_config):
        """
        ...
        # Build MOA layers
        # moa_fc = build_fc_layers(model_config, conv_out, "cont")

        return tf.keras.Model(inputs, [actor_critic_fc], name="CONT_Encoder_Model")
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        actor_critic_fc_output, cont_fc_output = self.moa_encoder_model(input_dict["obs"]["curr_obs"])
        ...
        new_state.extend([action_logits, cont_fc_output])

        self.compute_contribute_reward(input_dict, state[4], counterfactuals)
    ...
    def forward_rnn(self, input_dict, state, seq_lens):
        ...
        prev_moa_trunk = input_dict["prev_cont_trunk"]
